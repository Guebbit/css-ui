...
├── vuetify/tools/
│   └── _variant.scss       ← variant-selector() → generates &.button-{flat,outlined,...}, &--variant-*
└── atoms/buttons/
    └── simple-button/
...
| `&.button-rounded` | `&.button-rounded { … }` | `simple-button/index.scss` |
| `v-tools.variant-selector("flat")` | `&.button-flat, &--variant-flat { … }` | `src/vuetify/tools/_variant.scss` |
...
@use "src/vuetify/tools" as v-tools;

.my-button {
  @include tools.button-size("lg") { font-size: 1.25em; }
  &.button-pill { border-radius: 5em; }
  @include v-tools.variant-selector("outlined") { border-style: solid; }
...
@use "mixins" as *;
@use "../../../tools" as tools;            // size + interaction-states
@use "../../../vuetify/tools" as v-tools;  // variant-selector
...
.my-button {
  ...
  &.button-rounded  { border-radius: 28%; … }
  &.button-circular { border-radius: 50%; … }
  &.button-pill     { border-radius: 5em; … }

  // Colour/appearance variants (blueprint)
  @include v-tools.variant-selector("outlined") { border-style: solid; … }
  @include v-tools.variant-selector("plain")    { background: transparent; … }
...
| `&.button-rounded {}`, `&.button-circular {}` hard-coded | Same selectors kept inline where used |
| `&.button-flat {}`, `&.button-outlined {}` hard-coded | `@include v-tools.variant-selector("flat") {}` — shared mixin |
